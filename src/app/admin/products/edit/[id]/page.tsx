...
export default function EditProductPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  const [formData, setFormData] = useState({
    ...
    deliveryTime: product?.deliveryTime || "1-2 days",
    ...
    return (
      <div className="flex flex-col items-center justify-center min-h-[60vh]">
        ...
        <Link href="/admin/products">
          <button className="btn-primary mt-4">Back to Products</button>
        </Link>
      </div>
    );
  }
  ...
  if (showSuccess) {
    return (
      ...
      >
        ...
        <h2 className="text-2xl font-bold text-text mb-2">Product Updated!</h2>
    ...
    <div className="max-w-4xl mx-auto">
      ...
      >
        ...
        <div>
          ...
          <p className="text-text-muted">Update product details and affiliate links</p>
        </div>
      </motion.div>
          ...
          <div className="flex items-center gap-3">
            <button
              ...
              className={`relative w-14 h-7 rounded-full transition-colors ${
                formData.inStock ? "bg-accent-green" : "bg-surface-light"
              }`}
            >
              <div className={`absolute top-0.5 w-6 h-6 rounded-full bg-white shadow transition-transform ${
                formData.inStock ? "translate-x-7" : "translate-x-0.5"
              }`} />
            </button>
